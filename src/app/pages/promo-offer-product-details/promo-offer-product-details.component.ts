...
@Component({
  ...
  templateUrl: './promo-offer-product-details.component.html',
  styleUrls: ['./promo-offer-product-details.component.scss']
})
export class PromoOfferProductDetailsComponent implements OnInit, OnDestroy {
  // Data Form
  ...
  ngOnInit(): void {
    this.activatedRoute.paramMap.subscribe((param) => {
      this.promoSlug = param.get('slug');
      this.productSlug = param.get('slug2');
      console.log(this.promoSlug,this.productSlug)
      if (this.promoSlug && this.productSlug) {
        this.getProductByPromoSlug();
        this.getProductByIdOnlyDescription();
      }
    });
  ...
  private getProductByPromoSlug() {
    this.spinnerService.show();
    const select = 'name email username phoneNo gender role permissions hasAccess'
    this.subDataFive = this.promoOfferService.getProductDetailsByPromoandProduct(this.promoSlug, this.productSlug)
      .subscribe(res => {
        this.spinnerService.hide();
        if (res.success) {
          this.product = res.data?.products[0]?.product;
  ...
  private getProductByIdOnlyDescription() {
    this.spinnerService.show();
    // const select = 'name email username phoneNo gender role permissions hasAccess'
